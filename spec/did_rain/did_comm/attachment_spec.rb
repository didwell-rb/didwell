...
RSpec.describe DIDRain::DIDComm::Attachment do
  it "round-trips Base64 attachment" do
    a = DIDRain::DIDComm::Attachment.new(
      data: DIDRain::DIDComm::AttachmentDataBase64.new(base64: "dGVzdA=="),
      id: "att-1", description: "test attachment"
    )
    h = a.to_hash
    expect(h["data"]["base64"]).to eq("dGVzdA==")

    parsed = DIDRain::DIDComm::Attachment.from_hash(h)
    expect(parsed.data).to be_a(DIDRain::DIDComm::AttachmentDataBase64)
  ...
  it "round-trips JSON attachment" do
    a = DIDRain::DIDComm::Attachment.new(
      data: DIDRain::DIDComm::AttachmentDataJson.new(json: { "key" => "value" })
    )
    h = a.to_hash
    parsed = DIDRain::DIDComm::Attachment.from_hash(h)
    expect(parsed.data).to be_a(DIDRain::DIDComm::AttachmentDataJson)
    expect(parsed.data.json).to eq({ "key" => "value" })
  end

  it "round-trips Links attachment" do
    a = DIDRain::DIDComm::Attachment.new(
      data: DIDRain::DIDComm::AttachmentDataLinks.new(
    ...
    h = a.to_hash
    parsed = DIDRain::DIDComm::Attachment.from_hash(h)
    expect(parsed.data).to be_a(DIDRain::DIDComm::AttachmentDataLinks)
  ...
  describe "data_hash accessor" do
    it "uses data_hash for the hash JSON field" do
      data = DIDRain::DIDComm::AttachmentDataBase64.new(base64: "dGVzdA==", data_hash: "sha256-abc")
    ...
    it "round-trips hash field through JSON" do
      ...
      data = DIDRain::DIDComm::AttachmentDataBase64.from_hash(h)
    ...
    it "preserves Object#hash for use as Hash keys" do
      data1 = DIDRain::DIDComm::AttachmentDataJson.new(json: { "a" => 1 }, data_hash: "h1")
      data2 = DIDRain::DIDComm::AttachmentDataJson.new(json: { "b" => 2 }, data_hash: "h2")
    ...
    it "supports data_hash on AttachmentDataLinks" do
      data = DIDRain::DIDComm::AttachmentDataLinks.new(links: ["http://example.com"], data_hash: "sha-links")
  ...
  it "includes message with attachments" do
    msg = DIDRain::DIDComm::Message.new(
      type: "test", body: {},
      attachments: [
        DIDRain::DIDComm::Attachment.new(
          id: "123",
          data: DIDRain::DIDComm::AttachmentDataBase64.new(base64: "qwerty"),
          description: "abc",
          filename: "test.txt",
    ...
    parsed = DIDRain::DIDComm::Message.from_hash(h)
